...
\subsection{For free Photon fields}
Under the transformation
$$
A_{\mu} \rightarrow A_{\mu}^{\prime}=e^{-i \alpha} A_{\mu}
$$
where no conjugate transformation is for photon (since its anti-particle field is itself), \redp{the free electromagnetic Lagrangian following is not symmetric, and there is no 4-current for photons.}
$$
